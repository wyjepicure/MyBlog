...
{
    public abstract class BlogWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static BlogWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(BlogWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<BlogDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<BlogDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<BlogDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<BlogDbContext, Task<T>> func)
        {
            T result;
